...
const DrugCard = ({ drug }: { drug: IDrug }) => {
    ...
    return (
        <div className="flex flex-col cursor-pointer justify-center items-center space-y-3 bg-white border border-gray-200 hover:shadow-xl transition duration-700 ease-in-out transform hover:scale-105 p-4 box-border rounded-xl">
            ...
            {/* buttons */}
            <div className='flex'>
              <Link to={accessToken ? `/products/${drug.id}` : "/signin"} type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">View</Link>
