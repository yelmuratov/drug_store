...
import UpdateDrugModal from '@/modals/drug.update';

interface MyDrugItemProps {
  drug: IMyDrug;
  onDelete: (drugId: number) => void;
  onUpdate: (updatedDrug: IMyDrug[]) => void;
}

const MyDrugItem: React.FC<MyDrugItemProps> = ({ drug, onDelete, onUpdate }) => {
  ...
  return (
    <>
      <tr className="border-b dark:border-gray-700">
        ...
        <td className="px-4 py-3">{drug.brand}</td>
        <td className="px-4 py-3 max-w-[12rem] truncate">{drug.description}</td>
        <td className="px-4 py-3">${drug.price}</td>
        <td className="px-4 py-3 flex items-center justify-end">
          ...
          {isDropdownOpen && (
            <div id="dropdown-menu" className="absolute z-10 w-44 mt-40 md:mr-42 md:mr-[0px] mr-[180px] bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
